...
!ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
  subroutine  MYHX_DX(nType, N, X, Y)    
      ...
      do level = sF, 2, -1     ! Y0 = Y0 + H*X0, then update X0 and Y0 
         ...
         if (sNDVR .and. (level==sF)) then          
            call MG3X_Out_DX(level,level-1,nout(sF),outH,nin(sF),X0,    &
                             plen(sF-1),Y0)
         else
  	    if (sDEP(level)) then
	       if (myNode%nodNum(level)>1) then
                  call MG2X_Dep_DX(level,level-1,nout(level),           &
                                H0(myH0%mStart(level)),nin(level),      &
                                DEP(myDEP%pStart(level)),X0,plen(level-1),Y0)
               else
                  call MS2X_Dep_DX(level,level-1,nout(level),                 &
                                H0(myH0%mStart(level)),nin(level),blk(level), &
                                DEP(myDEP%pStart(level)),X0, plen(level-1),Y0)
               end if
            else             
  	       if (myNode%nodNum(level)>1) then
                  call MG1X_XYZ_DX(level,level-1,nout(level),                &
                                H0(myH0%mStart(level)),nin(level),X0,        &
                                plen(level-1),Y0 )
               else
                  call MS1X_XYZ_DX(level,level-1,nout(level),                &
                                H0(myH0%mStart(level)),nin(level),           &
      ...
      level = 1
      call H1X_XYZ_DX_Seq(nin(level),nout(level),blk(level),       &
                      H0(myH0%mStart(level)),X0,tmp)
