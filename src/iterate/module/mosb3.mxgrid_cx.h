...
!cccccccccccccccccccccccccccccccccccccccccccccccccccccccc
subroutine MG1X_XYZ_CX(s1, s2, nout1, xH1, nin1, X, nlen2, Y)
   integer, intent(IN) :: s1, s2, nin1, nout1, nlen2
   double complex, intent(IN)    :: xH1(nout1,nout1)
   double complex, intent(INOUT) :: X(nin1,nout1)
   double complex, intent(INOUT) :: Y(nin1,nout1)


   double complex :: X0(nin1*nout1), X1(nlen2)
   double complex :: Y0(nin1*nout1), Y1(nlen2), tmp(nin1)
...
!ccccccccccccccccccccc   redistribute X  cccccccccccccccccccccccccccccccccc
   ...
   do i = 1, recvNum
      if (nInd2(i)/=id) then                                     
         rNum2 = rNum2 + 1
         call MPI_IRecv(X1(locInd2(i)),lenInd2(i),MPI_DOUBLE_COMPLEX,   &
                  nInd2(i),gIndX2(i),MPI_COMM_WORLD, reqX2(rNum2),ierr)         
      end if
   end do

   ! redistribute X: sending data
   call copyVecCX(nin1*nout1, X, X0)
   ...
   do i = 1, sendNum     
      if (nInd1(i)==id) then   ! local copy
         ...
         rNum1 = rNum1 + 1
         call MPI_ISend(X0(locInd1(i)),lenInd1(i),MPI_DOUBLE_COMPLEX,  &
...
!cccccccccccccccc Y=Y+H*X, redistribute Y  cccccccccccccccccccccccccccccccc
   ...
   do i = 1, recvNum
      if (nInd2(i)/=id) then                                     
         rNum2 = rNum2 + 1
         call MPI_IRecv(Y1(locInd2(i)),lenInd2(i),MPI_DOUBLE_COMPLEX,   &
      ...
      do ii = 1, sendNum     
         if (gridInd(ii) == i) then
            if (nInd1(ii)==id) then   ! local copy
               ...
               rNum1 = rNum1 + 1
               call MPI_ISend(Y0(locInd1(ii)),lenInd1(ii),MPI_DOUBLE_COMPLEX, &
   ...
   call copyVecCX(nlen2, X1, X)
   call copyVecCX(nlen2, Y1, Y)
...
!ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
subroutine MG2X_DEP_CX(s1, s2, nout1, xH1, nin1, xDep, X, nlen2, Y)
   integer, intent(IN) :: s1, s2, nin1, nout1, nlen2
   double complex, intent(IN)   :: xH1(nout1,nout1)
   double complex, intent(IN)   :: xDep(nin1)
   double complex, intent(INOUT) :: X(nin1,nout1)
   double complex, intent(INOUT) :: Y(nin1,nout1)
...
!ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
subroutine MG3X_Out_CX(s1, s2, nout1, xOutH, nin1, X, nlen2, Y)
   integer, intent(IN) :: s1, s2, nin1, nout1, nlen2
   double complex, intent(IN)    :: xoutH(nin1,nout1,nout1)
   double complex, intent(INOUT) :: X(nin1,nout1)
   double complex, intent(INOUT) :: Y(nin1,nout1)
