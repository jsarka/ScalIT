...
!cccccccccccccccccccccccccccccccccccccccccccccccccccccccc
subroutine MS1VX(s1, s2, nout1, xVi, nin1, nblk1, nlen2, X)
   ...
   double precision :: X0(nlen2)
   double precision :: Y0(nin1,nout1,nblk1),Y1(nin1*nout1*nblk1)
   ...
   call copyVec(nin1*nout1*nblk1,Y0,Y1)
   ...
   do ii = 1, sendNum     
      if (nInd1(ii)==id) then   ! local copy
         ...
         rNum1 = rNum1 + 1
         call MPI_ISend(Y1(locInd1(ii)),lenInd1(ii),MPI_DOUBLE_PRECISION, &
...
!cccccccccccccccccccccccccccccccccccccccccccccccccccccccc
subroutine MS1VTX(s1, s2, nout1, xVi, nin1, nblk1, nlen2, X)
   integer, intent(IN) :: s1, s2, nin1, nout1, nblk1,nlen2
   double precision, intent(IN)  :: xVi(nout1,nout1,nblk1)
   double precision, intent(INOUT) :: X(nin1,nout1,nblk1)
