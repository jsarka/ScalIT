...
integer function PISTF_CONV_DX_MPI(MYID, ROOTID, E0, ETOL, NTYPE, N, X,  &
                 startCNT, STEPcnt, stepeig, M, M_MAX, H0XCX, LinSolv,   &
...
!cccccccccccccccccccccccccccccccc  
  ...
  integer :: dbSize, cxSize, intSize, fh
...
!************************************ 
  ...
  do  while ( cnt_high <=M_MAX )
     ...
     if (NUM <= 0) then
          PISTF_CONV_DX_MPI = -CNT_HIGH;     return
     end if
     ...
     if (MYID == ROOTID) then
          ...
          call ZGEEV('N', 'N', NUM, HMAT, M_MAX, tmpEIG, VL, LDVL, VR, LDVR, &
                     WORK, LWORK, RWORK, PISTF_Conv_DX_MPI)

         if (PISTF_CONV_DX_MPI == 0) then      
...
!                   print *, 'eig:',oldEig
     ...
     PISTF_CONV_DX_MPI=CONVINT(2)

     if (CONVINT(1) == 1)   exit

     cnt_low  = cnt_high + 1
     if (cnt_low >= M_MAX) then
           pistf_conv_DX_MPI = - M_MAX
           exit
      end if
  ...
  if (PISTF_CONV_DX_MPI>0) then  ! store data
    ...
    if (myid==rootID) then
       offset=0;
       call MPI_File_Write_At(fh, offset, PISTF_CONV_DX_MPI, 1, &
       ...
       offset=offset+intSize
       call MPI_File_Write_At(fh, offset, tmpEig, PISTF_CONV_DX_MPI,&
                   MPI_DOUBLE_COMPLEX, MPI_STATUS_IGNORE, ierr)

       offset=offset+PISTF_CONV_DX_MPI*cxSize
       do i = 1, PISTF_CONV_DX_MPI
          call MPI_File_Write_At(fh, offset, HMat_old(1,i), PISTF_CONV_DX_MPI, &
                   MPI_DOUBLE_COMPLEX, MPI_STATUS_IGNORE, ierr)
          offset=offset+PISTF_CONV_DX_MPI*cxSize
       end do
    endif
    offset=2*intSize+(PISTF_CONV_DX_MPI+PISTF_CONV_DX_MPI**2)*cxSize
  ...
  if (myID==rootID) then
      if (PISTF_CONV_DX_MPI>0) call Reorder_CX('A', NDIFFTYPE,  M, eig)
  end if
...
!        write(99) PISTF_CONV_DX_MPI, N
!        write(99) tmpEig(1:PISTF_CONV_DX_MPI),     &
!                  HMat_old(1:PISTF_CONV_DX_MPI,1:PISTF_CONV_DX_MPI)
!        write(99) VJ(1:N, 1:PISTF_CONV_DX_MPI)
!        close(99)
!      end if
!
