...
!ccccccccccccccccccccccccccccccccccccccc
      ...
      integer  :: pist1_sx_mpi, pist2_sx_mpi,pist51_sx_mpi,pist52_sx_mpi
      logical  :: pistconverg
...
!     TESTTYPE = 1
     ...
     case(1)      
      if(pist1_sx_MPI(MYID, ROOTID, E0, ETOL, N1, X, EIGNUM, MAX_NUM,  &
     ...
     case(2)
      if(pist2_SX_MPI(MYID, ROOTID,E0, ETOL, N1, X, EIGNUM, MAX_NUM,  &
                       HMULT,  MySolvCX, EIG, RES)>0) THEN
          IF (MYID==ROOTID) THEN
              print *
              print *, ' Performing PIST using Faster Convergent Criterion'
     ...
     case(3)
      if(pist51_SX_MPI(MYID, ROOTID,E0, ETOL, N1, X, EIGNUM, MAX_NUM,  &
                       HMULT,  MySolvCX, EIG, RES)>0) THEN
         IF (MYID==ROOTID) THEN
              print *
    ...
    case (4:)
      if(pist52_SX_MPI(MYID, ROOTID, E0, ETOL, N1, X, EIGNUM, MAX_NUM,  &
                       HMULT,  MySolvCX, EIG, RES)>0) THEN
         IF (MYID==ROOTID) THEN
            print *
            print *, ' Performing less strict PIST using Slower Convergent Criterion'
...
!c   Get the eigen values of H directly using ZGEEV
!c
      subroutine printEig0_SX(E0, num )
      integer,parameter :: N = 100
