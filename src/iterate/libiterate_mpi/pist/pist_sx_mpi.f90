...
integer function PIST_CONV_SX_MPI(MYID, ROOTID, E0, ETOL, NTYPE, N,  X,  &
...
!************************************ 
  ...
  do  while ( cnt_high <=M_MAX )
     ...
     NUM = PISTH0_SX_MPI(MYID, ROOTID, N, X,  M_MAX,  &
                CNT_LOW, CNT_HIGH, H0XCX, LinSolvCX, VJ, HMAT_OLD)

     if (NUM <= 0) then
          PIST_CONV_SX_MPI = -CNT_HIGH
          return
     end if
     ...
     if (MYID == ROOTID) then
          ...
          call ZGEEV('N', 'N', NUM, HMAT, M_MAX, tmpEIG, VL, LDVL, VR, LDVR, &
                     WORK, LWORK, RWORK, PIST_Conv_SX_MPI)

         if (PIST_CONV_SX_MPI == 0) then      
...
!                   print *, 'eig:',oldEig
     ...
     PIST_CONV_SX_MPI=CONVINT(2)

     if (CONVINT(1) == 1)   exit

     cnt_low  = cnt_high + 1
     if (cnt_low >= M_MAX) then
           pist_conv_SX_MPI = - M_MAX
           exit
      end if
...
!c         Apply PIST until M eigen values around E0 convergence          c
!cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
integer function PIST_CONVERG_SX_MPI(MYID, ROOTID, E0, ETOL, NTYPE, N,  X, &
                 startCNT,STEPcnt,stepeig,M,M_MAX,H0XCX,LinSolvCX,EIG,RES)
  implicit none
  include 'mpif.h'
  integer, intent(IN)          :: MYID, ROOTID
...
!cccccccccccccccccccccccccccccccc
  integer           :: PIST_EIG_SX_MPI
  double precision  :: getDiffMax_CX, GETSEPMAX_CX
   
!************************************   
  if ((stepEig >= M_MAX) .or. (startCnt > M_MAX) .or. (M > M_MAX)) then
      PIST_CONVERG_SX_MPI = 0
      return
  end if
  ...
  do  while ( CNT1 <=M_MAX )
      ...
      PIST_CONVERG_SX_MPI = PIST_EIG_SX_MPI(MYID, ROOTID, N, X,    &
             CNT1, CNT2, H0XCX, LinSolvCX,tmp1EIG,tmp2Eig)  

      if (PIST_CONVERG_SX_MPI <= 0) then
          PIST_CONVERG_SX_MPI = - CNT1
          return
      end if
...
!ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
!
integer function PIST_SX_MPI(MYID, ROOTID, N, X, M, H0X, LinSolv, EIG)
  implicit none
  include 'mpif.h'
  integer, intent(IN)  :: MYID, ROOTID
...
!cccccccccccccccccccc Local variables
  ...
  integer :: PISTHIJ_SX_MPI, INFO, IERR
  integer, parameter   ::  LDVL = 1, LDVR = 1,SCALE = 3
  ...
  PIST_SX_MPI = PISTHIJ_SX_MPI(MYID, ROOTID, N, X, M, H0X, LinSolv, HMAT)

  if ( (PIST_SX_MPI > 0) .and. (MYID==ROOTID) ) then
...
!ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
!
integer function PIST_EIG_SX_MPI(MYID,ROOTID,N,X,M1,M2,H0X,LinSolv,EIG1,EIG2)
  implicit none
  include 'mpif.h'
  integer, intent(IN)  :: MYID, ROOTID
...
!ccccccccccccccc  Local Variables cccccccccccccccccccccccccc  
  ...
  PIST_EIG_SX_MPI = PISTHIJ_SX_MPI(MYID,ROOTID,N,X,M_MAX,H0X,LinSolv,HMAT)

  if ( PIST_EIG_SX_MPI > 0 .and. (MYID == ROOTID)) then
      ...
      if (INFO /= 0) then
          ...
          call ZGEEV('N', 'N', M2, HOLD, M_MAX, EIG2, VL, LDVL, VR, LDVR, WORK, &
                   LWORK, RWORK, INFO)
           if (INFO /= 0)     PIST_EIG_SX_MPI = - M2           
      end if
  end if

  call MPI_BCAST(PIST_EIG_SX_MPI, 1, MPI_INTEGER, ROOTID, MPI_COMM_WORLD, IERR)
