...
!cccccccccccccccccccccccccccccccc
  ...
  double precision, dimension(M_MAX) :: tmpEig, oldEig2, Eig2, tmpEig2
  double precision, dimension(M)     :: oldEig  
  
  integer   :: cnt_high, cnt_low, cnt_start
  integer   :: I, J, NUM,IERR, II
...
!ccccccccccccc   Parameters to call SSTEV subroutine cccccccccccccc
 ...
 1002 format (E25.15)
 1001 format (2x,I3,F15.10,2x,F15.10)
 1003 format (2x,I3,F15.10,2x,F15.10,2x,F15.10)
 1004 format (2x,I3,F15.10,2x,F15.10,2x,F15.10,2x,F15.10)
!************************************ 
 if ((stepEig >= M_MAX) .or. (startCnt > M_MAX) .or. (M > M_MAX)) then
  ...
  do  while ( cnt_high <=M_MAX )   
     ...
     NUM = PISTH0_MPI(MYID, ROOTID, N, X,     &
               M_MAX, CNT_LOW, CNT_HIGH, H0X, LinSolv, VJ, HMAT_OLD, num2)
     ...
     if (myid==rootID) then
         print *, ' MPI Time for PIST:',wtp2-wtp1,' iterations:', num2
         print *
     end if
     ...
     if (MYID == ROOTID) then
         ...
         if (PISTF_CONV_MPI == 0) then      
              ...
              if (PISTF_CONV_MPI == 0) then  
                   ...
                   print *
                   !print 1002, eig(1:M)!, eig(1:M)-oldEig(1:M)
                   do II = 1, M
                      print 1001, II, eig(II), eig(II)-oldEig(II)
                   end do
                   print *
  ...
  if (MYID == ROOTID) then
   ...
   call DSYEV('N', 'U', NUM, HMAT, M_MAX, tmpEIG, WORK,    &
            LWORK, PISTF_Conv_MPI)
   call getWindow(E0, NUM, tmpEig, NUM, OLDEig2)

   NUM = cnt_high
   HMAT(1:NUM,1:NUM) = HMAT_old(1:NUM,1:NUM)
   call DSYEV('N', 'U', NUM, HMAT, M_MAX, tmpEIG,     &
           WORK, LWORK, PISTF_Conv_MPI)
   call getWindow(E0, NUM, tmpEig, NUM, Eig2)

   print *, "Converged states"
   !print 1002, eig(1:M)!, eig(1:M)-oldEig(1:M)
   do II = 1, NUM ! M
    tmpEig2(II) = eig2(II)-oldEig2(II)
    if ( dabs(tmpEig2(II)) <= ETOL*100 ) then
      print 1004, II, Eig2(II), Eig2(II)-oldEig2(II)
    end if
   end do
   print *
  ...
  if (MYID == ROOTID) then
      print *, 'Total number of iterations (Lanczos*QMR) :', CNT_HIGH, num3
      print *
  end if
  ...
  if (PISTF_CONV_MPI>0) then  ! store data
    ...
    if (myid==rootID) then
       print *, 'Storing data in sPT file!'
       offset=0;
    ...
    if (myID==rootID) then
      ...
      print *, 'MPI Time for Storing:',wtp4-wtp3
    end if
