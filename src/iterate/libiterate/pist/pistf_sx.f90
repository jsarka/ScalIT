...
integer function PISTF_CONV_SX(E0, ETOL, NTYPE, N, X, startCNT, stepCnt,  &
...
!***************************************************** 
  ...
  do  while ( cnt_high <=M_MAX )
     ...
     if ( NUM <= 0) then
         PISTF_CONV_SX = -CNT_HIGH;    return
     end if  

     num = cnt_high-stepEig
     HMAT(1:num,1:num) = HMAT_OLD(1:num,1:num)  
     call ZGEEV('N', 'N', NUM, HMAT, M_MAX, tmpEig, VL, LDVL, VR, LDVR,    &
                 WORK, LWORK, RWORK, PISTF_Conv_SX)
     if (PISTF_CONV_SX /= 0) then       ! Lapack error   
         PISTF_CONV_SX = -NUM;        return                
     end if
     ...
     if ( RES <= ETOL ) then
          PISTF_CONV_SX = cnt_high;      exit
     end if    

     PISTF_CONV_SX = - cnt_high 

     cnt_low  = cnt_high + 1
     if (cnt_low >= M_MAX) then
           pistf_conv_SX = - M_MAX;        exit
      end if
  ...
  if (PISTF_CONV_SX>0) then  ! store data
     open(99, FILE=filename, status='Replace', FORM='UNFORMATTED')
     write(99) PISTF_CONV_SX, N
     write(99) tmpEig(1:PISTF_CONV_SX),HMat_old(1:PISTF_CONV_SX,1:PISTF_CONV_SX)
     write(99) VJ(1:N, 1:PISTF_CONV_SX)
     close(99)
  end if

  call Reorder_CX('A', nDiffType,  M, eig)
  
end function PISTF_CONV_SX
!**************************************************************************
