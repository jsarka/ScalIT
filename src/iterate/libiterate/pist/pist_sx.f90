...
!cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
integer function PIST_CONV_SX(E0, ETOL, NTYPE, N, X, startCNT,   &
...
!************************************ 
  ...
  do  while ( cnt_high <=M_MAX )
     ...
     NUM=PISTH0_SX(N,X,M_MAX,CNT_LOW,CNT_HIGH,H0XCX,LinSolvCX,VJ,HMAT_OLD)

     if ( NUM <= 0) then
         PIST_CONV_SX = -CNT_HIGH;   return
     end if  

     num = cnt_high-stepEig
     HMAT(1:num,1:num) = HMAT_OLD(1:num,1:num)  
     call ZGEEV('N', 'N', NUM, HMAT, M_MAX, tmpEig, VL, LDVL, VR, LDVR,    &
                 WORK, LWORK, RWORK, PIST_Conv_SX)
     if (PIST_CONV_SX /= 0) then       ! Lapack error   
         PIST_CONV_SX = -NUM;    return                
     end if
     ...
     if ( RES <= ETOL ) then
          PIST_CONV_SX = cnt_high;        exit
     end if    

     PIST_CONV_SX = - cnt_high 

     cnt_low  = cnt_high + 1
     if (cnt_low >= M_MAX) then
           pist_conv_SX = - M_MAX;      exit
      end if
...
!c         Apply PIST until M eigen values around E0 convergence          c
!cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
integer function PIST_CONVERG_SX(E0, ETOL, NTYPE, N, X, startCNT,   &
                  STEPcnt, stepeig, M, M_MAX, H0XCX, LinSolvCX, EIG, RES)
  implicit none
  double precision, intent(in) :: E0, ETOL
  integer, intent(IN)  :: N, NTYPE
...
!************************************  
  ...
  if ((stepEig >= M_MAX) .or. (startCnt > M_MAX) .or. (M > M_MAX))  then
      PIST_CONVERG_SX = 0;      return
  end if

  Cnt1  = max(startCnt, M, stepEig+1) 

  PIST_CONVERG_SX = - M_MAX

  do  while ( CNT1 <=M_MAX )
      CNT2    = CNT1-STEPEIG

      PIST_CONVERG_SX=PIST_EIG_SX(N,X,CNT1,CNT2,H0XCX,LinSolvCX,tmp1EIG,tmp2Eig)

      if (PIST_CONVERG_SX <= 0) then
          PIST_CONVERG_SX = -CNT1;        return
      end if
...
integer function PIST_SX(N, X, M, H0X, LinSolvCX, EIG)
  implicit none
...
!ccccccccccccccc  Local Variables cccccccccccccccccccccccccc  
  ...
  PIST_SX = PISTHIJ_SX(N, X, M, H0X, LinSolvCX, HMAT)
...
!ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
integer function PIST_EIG_SX(N, X, M1, M2, H0X, LinSolv, EIG1, EIG2)
  implicit none
...
!ccccccccccccccc  Local Variables cccccccccccccccccccccccccc  
  ...
  PIST_EIG_SX = PISTHIJ_SX(N, X, M_MAX, H0X, LinSolv, HMAT)

  if ( PIST_EIG_SX > 0 ) then
