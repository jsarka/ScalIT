...
integer function pistHij_cx(N, X, M, H0X, LinSolv, HMAT)
   implicit none
   ...
   do J = 1, M-1          ! WJ = VJ(1:N, J) / (H-EI)
      ...
      if (NUM <= 0) then       
         PISTHIJ_CX = num;     return
      end if       

      if (.not. MGS_ORTH_CX(N, WJ, J, VJ)) then        
         PISTHIJ_CX = -J;       return        
...
integer function pistH0_CX(N, X, M_MAX, M_LOW, M_HIGH, H0X, LinSolv, VJ, HMAT)
   implicit none
   ...
   do J = M_LOW, M_HIGH
      call H0X(N, VJ(1:N, J), WJ)         ! WJ = H * VJ
      do I = 1, M_HIGH
         HMAT(I, J)  = dot_product(VJ(1:N,I), WJ)  ! HMAT(I,J) = <UI|H|UJ>        
      end do
   end do
