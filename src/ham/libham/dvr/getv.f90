...
logical function getPISTV(filename, N, NS, V)
   ...
   if ((N0/=N).OR.(NP<NS)) then
      WRITE(*,*) "Problem with allocation!"
      IF (N0/=N) THEN
          WRITE(*,*) "Vector size missmatch!"
          WRITE(*,*) "File's N = ", N0, "Input N = ", N
      ELSE IF (NP<NS) THEN
          WRITE(*,*) "Number of states requested is more than files number of states avaialble."
          WRITE(*,*) "States requested = ", NS, "States available = ", NP
      END IF 
      close(99); return
   end if
...
!cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
!c              Get the PIST wavefunction                       c
!c  This version is called when the user specifies a certain    c
!c    index of states that are desired.    -C.P.                c
!cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc

logical function getPISTV_Index(filename, N, NS, NSind, V)
   implicit none
   character(len=*), intent(IN) :: filename  ! Output file from iterate step that stores PIST vectors
   integer, intent(IN) :: N                  ! Length of each vector, as inputed by user
   INTEGER, INTENT(IN) :: NS                 ! Number of states desired, as inputed by user
   INTEGER, INTENT(IN) :: NSInd(NS)          ! array of the desired state's indices
   double precision, intent(OUT) :: V(N, NS) ! outputted array of read PIST vectors in order 
                                             !   of desire indices.
