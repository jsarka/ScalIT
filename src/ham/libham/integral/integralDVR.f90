...
!cccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
!cccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
logical function integralDVR(N, S1, X1, X0)
   implicit none
   include 'dvrData.h'
   integer, intent(IN) :: N
   ...
   integralDVR = .false.;     lwork=BKSIZE*N
...
!cccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
!cccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
logical function integralDVRF(N, S1, X1, X0, wrFlag, fname)
   implicit none
   include 'dvrData.h'
   integer, intent(IN) :: N
   double precision, intent(INOUT) :: S1(N,N), X1(N,N)
   ...
   integralDVRF = .false.;     lwork=BKSIZE*N
   allocate(X2(N,N), tmp(N,N), work(lwork), Y0(N), stat=info)   
   if (info/=0)  return
   ...
   if (wrFlag) then    ! save transformation matrices in file
      ...
      if (info==0) then
          ...
          close(99);       integralDVRF = (N0==N)
      end if
   end if 

   if (integralDVRF) then
      do i = 1, N
         tmp(1:N, i) = S1(1:N,i)*Y0(i)
      end do
...
!cccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
!cccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
logical function integralDVRCx(N, S1Cx, X1Cx, X0)
   implicit none
   include 'dvrData.h'
   integer, intent(IN) :: N
   ...
   integralDVRCx = .false.;     lwork=BKSIZE*N
...
!cccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
!cccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
logical function integralDVRCxF(N, S1Cx, X1Cx, X0, wrFlag, fname)
   implicit none
   include 'dvrData.h'
   integer, intent(IN) :: N
   double complex, intent(INOUT) :: S1Cx(N,N), X1Cx(N,N)
   ...
   integralDVRCxF = .false.;     lwork=BKSIZE*N;  
   allocate(X2Cx(N,N),tmp(N,N),work(lwork),rwork(lwork),Y0(N),stat=info)   
   if (info/=0)  return
   ...
   if (wrFlag) then    ! save transformation matrices in fil
       ...
       if (info==0) then          ! save matrices
          ...
          close(99);   integralDVRCxF=(info==0)
       end if
   else
      open(99,file=fname,status='Old',form='unformatted',iostat=info)
      ...
      if (info==0) then
          ...
          close(99);    integralDVRCxF=(N==N0)
       end if
   end if

   if (integralDVRCxF) then
      do i = 1, N
         tmp(1:N, i) = S1Cx(1:N,i)*Y0(i)
      end do
