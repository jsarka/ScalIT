...
PROGRAM TEST_gaunt
   ...
   integer :: getGauntJMSize, getGauntCoeffSize, getGauntCoeffSize1
   integer :: jmSize, jfSize, jffSize
   logical :: s1
   integer ::  I, OPT = 1

   DO WHILE (OPT /= 0)
       PRINT *, 'Input jmax for Gaunt symbol: j1, j2, j3 '
       read  *, jmax(1:3)
       jmSize = getGauntJMSize(jmax) 
       jfsize = getGauntCoeffSize(jmax)
       print *, 'Size of non-zero Gaunt coefficients. Total:',jmSize, jfSize

       print *, 'Whether to Show Length and base: T=Show, F=Not Show'
       read *, s1
       if (s1) then
          allocate(jLen0(jmSize), jBase(jmSize))
          jffSize = getGauntCoeffSize1(jmax,jmSize, jLen0, jBase)
          print *
       ...
       if (s1) then
           ...
           call getGauntLen   (jmax, jmSize, JLen1)
           call getGauntLen0  (jmax, jmSize, JLen2)
          print *, 'jLen2', jLen2         
           call getGauntIndex (jmax, jmSize, jInd1)
           call getGauntIndex0(jmax, jmSize, jind2)
           call getGauntLenInd(jmax, jmSize, jLen3, jInd3)
           call getGauntLenInd0(jmax, jmSize, jLen4,jind4)
